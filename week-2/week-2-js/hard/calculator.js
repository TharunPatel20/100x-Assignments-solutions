...
class Calculator {
  ...
  calculate(expression) {
    ...
    if (/[^0-9+\-*/().]/.test(noSpaceString)) {
      throw new Error("invalid input");
    }
    const stack = [];

    for (const char of noSpaceString) {
      if (char === "(") {
        stack.push(char);
      } else if (char === ")") {
        if (stack.length === 0) {
    ...
    try {
      const ans = new Function("return " + noSpaceString)();
      if (!isFinite(ans)) {
        throw new Error("");
      }
      this.result = ans;
...
//     // Validate the expression: Only allow numbers, operators, and parentheses
